...
import ch.unisg.ics.interactions.hmas.core.io.AbstractGraphReader;
...
public class SignifierGraphReader extends AbstractGraphReader {
  ...
  private Signifier readSignifier() {
    Optional<Resource> bSpecNode = Models.objectResource(model.filter(signifierIRI, SIGNIFIES, null));
    ...
    if (bSpecNode.isPresent()) {
      ...
      if (signifierIRI.isIRI()) {
        builder.setIRIAsString(signifierIRI.stringValue());
      }

      Set<Resource> abilities = Models.objectResources(model.filter(signifierIRI, RECOMMENDS_ABILITY,
              null));
      ...
      Models.objectLiteral(model.filter(signifierIRI, RDFS.LABEL, null))
              .ifPresent(l -> builder.setLabel(l.getLabel()));
      Models.objectLiteral(model.filter(signifierIRI, RDFS.COMMENT, null))
              .ifPresent(l -> builder.setComment(l.getLabel()));

      Set<Resource> contexts = Models.objectResources(model.filter(signifierIRI, RECOMMENDS_CONTEXT, null));
      contexts.forEach(c -> {
      ...
      if (signifierIRI.isIRI()) {
        builder.setIRI(SimpleValueFactory.getInstance().createIRI(signifierIRI.stringValue()));
      }
      return (Signifier) readResource(builder, signifierIRI);
    } else {
